...
- **CI (`.github/workflows/CI.yaml`) runs backend `pytest` plus a frontend job** (`npm run lint` via oxlint, then `npm run build`) — added 2026-08-01. There is still no frontend automated *test* step (no Vitest/RTL suite exists to run — see below), only lint + build/type-check coverage.
- **Branding is "Gorilla" everywhere** — settled 2026-07-27, internal identifiers finished 2026-07-28. The UI header, `index.html` title, PWA manifest, frontend `README.md`, `package.json`'s `name` (`gorilla`) and the FastAPI app title (`Gorilla API`) all say Gorilla; anything still reading "Żelazo" or "Gym Tracker" is stale. The localStorage token key is now `gorilla_token`; `lib/api-client.js` carries a one-time migration that moves any pre-existing `zelazo_token` value over on load so the rename didn't log anyone out — that migration block is safe to delete once pre-rebrand sessions have expired. The one thing deliberately *not* renamed is the local Postgres database/role (`gym_tracker` / `gym_user`): it's infrastructure, not branding, and renaming it means recreating the DB and rewriting every developer's `.env`.
...
- **`Button`'s `pulseOnClick` confirmation-pulse is fixed** (2026-07-28) — it previously didn't turn the button green at all, because the pulse color class was merely *appended* after the variant's base `bg-*` class and Tailwind's generated CSS order, not className string order, decides the cascade. Now the pulse classes carry Tailwind v4's important suffix (`bg-success! hover:bg-success! text-accent-fg!`), which wins regardless of order. Re-verified by measured `getComputedStyle` in headless Chromium: base `rgb(255,255,130)` → `rgb(124,144,130)` during the pulse → back after. If you touch this, keep the `!` — dropping it silently reintroduces the original bug, and it's invisible in code review.

## Palette

Set 2026-07-28 from a coolors palette the user picked (`coolors.co/7c9082-9d69a3-ffff82-2f2f2f-f2f4ff`), briefed as "mainly black and yellow". All of it lives in `@theme` in `frontend/src/styles/globals.css` — **no component should hardcode a hex**; the four that used to (`Button`, `ErrorBanner`, `WorkoutCard`) were converted, and a stray hex is now a bug, not a style choice.

- Black `#1B1B1B` base, Graphite `#2F2F2F` surfaces, Ghost White `#F2F4FF` text, Canary Yellow `#FFFF82` accent. Dusty Olive `#7C9082` is `--color-success` (the pulse). Vintage Lavender is deliberately unused — there was no role for it that didn't fight the black/yellow brief.
